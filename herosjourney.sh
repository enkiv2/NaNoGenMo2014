...
location() {
	echo -e {a,the}\ {cave,cavern,mansion,castle,tower,shack,riverbed,gutter,forest,valley,trees,wall,shipyard,junkyard}\ {at\ the\ edge\ of,under,behind,above,beneath,a\ few\ miles\ away\ from,beside}\ {town,his\ house,his\ school,his\ grandmother\'s\ house,his\ crazy\ uncle\'s\ house,the\ old\ mill,the\ forbidden\ zone,$HOMETOWN}\\n | sort -R | head -n 1
}
intro=$(echo -e Once{\,\ long\ ago,\ upon\ a\ time,},\ in\ {the\ city\ of,the\ town\ of,the\ land\ of,the\ hamlet\ of,the\ village\ of}\ $HOMETOWN,\ there\ {lived,resided}\ $HERO." $(weather)".\ He\ {first\ ,}{found,discovered,saw,came\ upon}\ $WEAPON\ in\ "$(location)."\\n"$(weather) in "{the\ city\ of,the\ town\ of,the\ land\ of,the\ hamlet\ of,the\ village\ of}\ $HOMETOWN\ when\ $HERO\ {first\ ,}{found,discovered,saw}\ $WEAPON.\ It\ was\ in\ "$(location)"\\n | sort -R | head -n 1)
